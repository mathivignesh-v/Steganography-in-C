...
//byte to lsb
Status encode_byte_to_lsb(char data, char *image_buffer)   // Function to hide 1 byte (data) into 8 image bytes using LSB method
{
    for (int i = 0; i < 8; i++)   // Loop runs 8 times because 1 character = 8 bits
    {
        image_buffer[i] = image_buffer[i] & ~1;   // Clear the Least Significant Bit (LSB) of current image byte

        char bit = (data >> (7 - i)) & 1;   // Extract one bit from data (starting from MSB to LSB)

        image_buffer[i] = image_buffer[i] | bit;   // Insert the extracted bit into the LSB of image byte
    }
    return e_success;   // Return success status after encoding
}
...
Status encode_magic_string(const char *magic_string, EncodeInfo *encInfo)
{
    ...
    {  
        ...
        encode_byte_to_lsb(MAGIC_STRING[i], buffer);   //encode 1 character into those 8 bytes
...
Status copy_remaining_img_data(FILE *fptr_src, FILE *fptr_dest)   // Function to copy remaining image data from source file to destination file
{
    char buffer[1024];   // Create a buffer of 1024 bytes to store data temporarily

    size_t bytes;   // Variable to store number of bytes read from source file

    while ((bytes = fread(buffer, 1, sizeof(buffer), fptr_src)) > 0)   // Read data from source file until EOF
    {
        fwrite(buffer, 1, bytes, fptr_dest);   // Write the same number of bytes into destination file
    }

    return e_success;   // Return success after copying all remaining data
}
...
Status do_encoding(EncodeInfo *encInfo) 
{
    ...
    printf("Step 1: Opening required files...\n");
    ...
    printf("Step 2: Checking image capacity...\n");
    ...
    printf("Step 3: Copying BMP header...\n");
    ...
    printf("Step 4: Encoding magic string...\n");
    ...
    printf("Step 5: Encoding secret file extension size...\n");
    ...
    printf("Step 6: Encoding secret file extension...\n");
    ...
    printf("Step 7: Encoding secret file size...\n");
    ...
    printf("Step 8: Encoding secret file data...\n");
    ...
    printf("Step 9: Copying remaining image data...\n");
...
Status encode_int_to_lsb(int size, char *buffer)
{
    for (int i = 0; i < 32; i++)
    {
        ...
        // Extract bit from size (MSB first)
